...
import { ArrowLeft, Activity, Heart, Thermometer, Droplet, Wind, AlertTriangle, CheckCircle2 } from 'lucide-react'

// Protocolo de Manchester Simplificado
const NIVEIS_RISCO = [
  { id: 'EMERGENCIA', label: 'Emergência (0 min)', color: 'bg-rose-600 hover:bg-rose-700 text-white border-rose-700' },
  { id: 'MUITO_URGENTE', label: 'Muito Urgente (10 min)', color: 'bg-orange-500 hover:bg-orange-600 text-white border-orange-600' },
  { id: 'URGENTE', label: 'Urgente (60 min)', color: 'bg-amber-400 hover:bg-amber-500 text-amber-950 border-amber-500' },
  { id: 'POUCO_URGENTE', label: 'Pouco Urgente (120 min)', color: 'bg-emerald-500 hover:bg-emerald-600 text-white border-emerald-600' },
  { id: 'NAO_URGENTE', label: 'Não Urgente (240 min)', color: 'bg-blue-500 hover:bg-blue-600 text-white border-blue-600' }
]
...
export function TriagemPage() {
  ...
  const salvarTriagem = async (e: FormEvent) => {
    ...
    try {
      // Simulação: Aqui chamaria um endpoint para salvar a triagem e depois atualizar o status
      await atendimentoService.atualizarStatus(Number(id), 'AGUARDANDO_ATENDIMENTO')
      ...
      <div className="flex items-center gap-4 border-b border-border pb-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm flex items-center gap-2 mt-1">
            ...
            <span>•</span>
            <span className="text-amber-600 flex items-center gap-1"><AlertTriangle className="h-3 w-3" /> Alergia a Dipirona</span>
          </p>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div className="border border-border rounded-lg bg-card p-3 shadow-sm flex flex-col justify-between">
             ...
             {/* Peso */}
             <div className="border border-border rounded-lg bg-card p-3 shadow-sm flex flex-col justify-between">
              <div className="text-xs font-semibold uppercase text-muted-foreground mb-2 flex items-center gap-1">Peso (kg)</div>
              <input type="number" step="0.1" className="w-full h-10 px-2 font-mono text-lg rounded border border-input bg-background focus:ring-2 focus:ring-primary focus:outline-none" placeholder="75.5" value={form.peso} onChange={e => setForm({...form, peso: e.target.value})} />
            </div>
